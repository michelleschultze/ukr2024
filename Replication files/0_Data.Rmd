...
master_dataset <- master_dataset %>%
  mutate(Country.Name = case_when(
    ...
    Country.Name == "Venezuela" ~ "Venezuela, RB",
    Country.Name == "Turkey" ~ "Turkiye",
    Country.Name == "Hong Kong" ~ "Hong Kong SAR, China",
    Country.Name == "Macau" ~ "Macao SAR, China",
    Country.Name == "Brunei" ~ "Brunei Darussalam",
    Country.Name == "West Bank and Gaza" ~ "Palestine",
    Country.Name == "Russia" ~ "Russian Federation",
    Country.Name == "Saint Martin" ~ "St. Martin (French part)",
...
duplicate_rows <-master_dataset %>%
  filter(Indicator.Name %in% c("fatalities_y, Violence against civilians", "fatalities_y, Strategic developments", "fatalities_y, Protests", "fatalities_y, Explosions/Remote violence", "fatalities_y, Battles", "count_events_y, Violence against civilians", "count_events_y, Strategic developments", "count_events_y, Protests", "count_events_y, Explosions/Remote violence", "count_events_y, Battles", "count_events_y, All types of violent conflict", "AirportsVolume", "AirportsCount", "GDP growth (annual %)", "Tax revenue (% of GDP)", "Inflation, GDP deflator (annual %)", "passengers_spike_true", "flights_spike_true", "total_passengers_in", "total_passengers_out", "total_flights_in", "total_flights_out", "total_freight_in", "total_freight_out", "Foreign direct investment, net inflows (% of GDP)"),
         year >= 1990) %>% #1990 for the 1990 dataset
  group_by(Country.Name, year, Indicator.Name) %>%
  summarise(n = n()) %>%
  filter(n > 1)
...
```{r specifications}
scm_data <- master_dataset %>%
  filter(Indicator.Name %in% c("fatalities_y, Violence against civilians", "fatalities_y, Strategic developments", "fatalities_y, Protests", "fatalities_y, Explosions/Remote violence", "fatalities_y, Battles", "count_events_y, Violence against civilians", "count_events_y, Strategic developments", "count_events_y, Protests", "count_events_y, Explosions/Remote violence", "count_events_y, Battles", "count_events_y, All types of violent conflict", "AirportsVolume", "AirportsCount", "GDP growth (annual %)", "Inflation, GDP deflator (annual %)", "passengers_spike_true", "flights_spike_true", "total_passengers_in", "total_passengers_out", "total_flights_in", "total_flights_out", "total_freight_in", "total_freight_out", "Foreign direct investment, net inflows (% of GDP)", "GDP (current US$)", "flights_in_change_pct", "passengers_in_change_pct", "Merchandise trade (% of GDP)", "Mineral rents (% of GDP)", "Services, value added (% of GDP)", "Agriculture, forestry, and fishing, value added (% of GDP)"),
         year >= 1990, year <= 2019) %>%     #1990 for the 1990 dataset
  pivot_wider(names_from = Indicator.Name, values_from = value) %>%
  group_by(Country.Name, year) %>%
  summarise_all(~ if (any(!is.na(.))) .[!is.na(.)][1] else NA)
#Note: you can add variables in the filter above and export the resulting dataset in order to add more columns. for instance, add: , "Manufacturing, value added (% of GDP)", "Merchandise trade (% of GDP)", "Imports of goods and services (% of GDP)", "Mineral rents (% of GDP)", "Services, value added (% of GDP)", "Agriculture, forestry, and fishing, value added (% of GDP)", "Trade (% of GDP)", "Trade in services (% of GDP)"
# had to kick out the trade and manufacturing% vars because they had NAs in weird spots
```
...
Iraq and El Salvador have weird gaps in the FDI variable in 1990 where 2 singular years don't have data (for each). This is consequential later at least for Iraq, so let's impute those variables as the last observation carried forward.
Also Turkmenistan has gaps in the %services variable.
```{r}
# Apply LOCF for Iraq, El Salvador (FDI), and Turkmenistan (services value added)
scm_data_imputed <- scm_data_imputed %>%
  arrange(Country.Name, year) %>%  # Ensure data is sorted by country and year
  group_by(Country.Name) %>%
  mutate(
    # LOCF for Iraq and El Salvador: FDI in the 1990s
    `Foreign direct investment, net inflows (% of GDP)` = if_else(
      (Country.Name %in% c("Iraq", "El Salvador")) & year >= 1990 & year < 2000,
      na.locf(`Foreign direct investment, net inflows (% of GDP)`, na.rm = FALSE),
      `Foreign direct investment, net inflows (% of GDP)`
    ),
    # LOCF for Turkmenistan: Services value added
    `Services, value added (% of GDP)` = if_else(
      Country.Name == "Turkmenistan",
      na.locf(`Services, value added (% of GDP)`, na.rm = FALSE),
      `Services, value added (% of GDP)`
    )
  ) %>%
  ungroup()

# Check the imputed values for Iraq, El Salvador, and Turkmenistan
scm_data_imputed %>%
  filter(Country.Name %in% c("Iraq", "El Salvador", "Turkmenistan")) %>%
  select(Country.Name, year, `Foreign direct investment, net inflows (% of GDP)`, `Services, value added (% of GDP)`)
```
...
write.csv(data, "full_data_1990.csv")
```
