...
sums %>%
  ...
  geom_point() +
  labs(title = "Logged FDI ranges compared", subtitle = "Nominal and real FDI differences are even more smoothed out through logs.")
```
...
No FE: 
(1a) FDI = pass + ε
log(total real FDI) = lag(percentage change in passengers flying in)
(2a) FDI = pass + ACLED + ε
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(ACLED battle deaths))
(3a)  FDI = pass + ACLED + real GDP + ε
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(real total GDP)) + lag(log(ACLED battle deaths))
(4a) FDI = pass + ACLED + real GDP per capita + ε 
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(ACLED battle deaths)) + lag(log(real GDP per capita, PPP))
(5ai) FDI = pass + ACLED + nominal GDP int$ + ε
log(total real FDI) = lag(percentage change in passengers flying in) +  lag(log(ACLED battle deaths)) + lag(log(nominal GDP in int$ constant 2017))
(5aii) FDI = pass + ACLED + nominal GDP US$ + ε
log(total real FDI) = lag(percentage change in passengers flying in) +  lag(log(ACLED battle deaths)) + lag(log(nominal GDP in USD current))
(6a) FDI = pass + ACLED + real GDP per capita + inflation deflator + ε
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(real total GDP)) + lag(log(ACLED battle deaths)) + lag(inflation_rate)
...
```
Model 1a is the most basic model, and we see the coefficient of interest is not significant. Heteroskedascity not rejected.
...
```
Model 2a adds battle deaths as a covariate. Passengers is not significant, and battle deaths is slightly significant at the 10% level. Joint significance not present, and we have weak evidence for homoskedascity.

*Adding different GDP measures*
(3a) FDI = pass + ACLED + real GDP + ε
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(real total GDP)) + lag(log(ACLED battle deaths))
...
summary(model.3a)
bptest(model.3a) #fail to reject null that variance of error term is constant.
```
Model 3a adds real GDP as a covariate. Passengers is not significant, but the coefficients are jointly significant.

(3b) FDI = pass + ACLED + real GDP per capita + ε 
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(ACLED battle deaths)) + lag(log(real GDP per capita, PPP))
```{r model.4a, message=TRUE, warning=TRUE}
model.3b <- lm(log_FDI_r ~ lag(passengers.in.change.pct) + lag(log_battle) + lag(log_GDP_r_pc), data = data)
summary(model.3b)
bptest(model.3b) #fail to reject null that variance of error term is constant. 
```
Instead of total real GDP, Model 3b adds per capita real GDP as a covariate. Joint significance still present, though the coefficient is not significant.


(3ci) FDI = pass + ACLED + nominal GDP int$ + ε
log(total real FDI) = lag(percentage change in passengers flying in) +  lag(log(ACLED battle deaths)) + lag(log(nominal GDP in int$ constant 2017))
```{r model.5a, message=TRUE, warning=TRUE}
model.3ci <- lm(log_FDI_r ~ lag(passengers.in.change.pct) + lag(log_battle) + lag(log1p(total_GDP_n_ID)), data = data)
summary(model.3ci)
bptest(model.3ci) #reject null that variance of error term is constant. homoskedascity likely present.
```
Not significant, but joint significance remains.

(3cii) FDI = pass + ACLED + nominal GDP US$ + ε
log(total real FDI) = lag(percentage change in passengers flying in) +  lag(log(ACLED battle deaths)) + lag(log(nominal GDP in USD current))
```{r model.3cii, message=TRUE, warning=TRUE}
model.3cii <- lm(log_FDI_r ~ lag(passengers.in.change.pct) + lag(log_battle) + lag(log1p(`GDP.current.US$`)), data = data)
summary(model.3cii)
bptest(model.3cii) #reject null that variance of error term is constant. homoskedascity likely present.
```
The sign on the coefficient flips, and the coefficient is even less significant. Joint significance remains... is this defunct?

(4a) FDI = pass + ACLED + real GDP per capita + inflation deflator + ε
log(total real FDI) = lag(percentage change in passengers flying in) + lag(log(real total GDP)) + lag(log(ACLED battle deaths)) + lag(inflation_rate)
```{r model.3cii, message=TRUE, warning=TRUE}
model.4a <- lm(log_FDI_r ~ lag(passengers.in.change.pct) + lag(log_battle) + lag(log_GDP_r) + lag(`Inflation.GDP.deflator.annual.pct`), data = data)
summary(model.4a)
bptest(model.4a) #reject null that variance of error term is constant. homoskedascity likely present.
```
Positive beta, joint significance remains when adding inflation deflator as a covariate.
...
```{r model.3a, message=TRUE, warning=TRUE}
model.3a <- lm(log_FDI_r ~ lag(passengers.in.change.pct) + lag(log_battle) + lag(log_GDP_n), data = data_Eur)
summary(model.3a)
bptest(model.3a) #fail to reject null that variance of error term is constant.
```
...
```{r model.4a, message=TRUE, warning=TRUE}
model.4a <- lm(log_FDI_r ~ lag(passengers.in.change.pct) + lag(log_battle) + lag(log_GDP_r), data = data_Eur)
summary(model.4a)
