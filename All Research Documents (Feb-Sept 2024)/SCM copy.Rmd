...
*because we eliminated the RD design, we don't need to have a symmetric dataset. Therefore we're not running the following chunk so that we can preserve the full range of data *
We are missing entire observations for 10 countries in certain years, if we shorten the time frame to 2012-2023. We're missing only four countries, if we shorten the window to 2014-2023. It might be worth it to shorten the window to 2014-2023, so let's drop those four countries.
...
```
^The above was edited because we removed the "na-obs" chunk to preserve the full range of data, therefore naming the new dataset "FULLDATA_NAs_May242024.csv".
